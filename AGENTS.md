...
**Recent additions** (since last map update, 2026-08-14): **speaker identity registry** (`speaker-identity-registry` change: global `speakers` registry, `speaker_embeddings` voiceprints with two-owner CHECK, `meeting_speakers` cluster→person mapping + centroids, `meeting_expected_speakers` allowlists; `audio/speaker_recognition.rs` cosine matcher τ=0.7; `database/repositories/speaker.rs`; commands `list_speakers`/`assign_speaker`/`rename_speaker`/`set_expected_speakers`/`assign_live_speaker`/`rematch_meeting_speakers`/`finalize_online_session`/`speaker_storage_stats`; live Fast-mode `PrototypeStore`), **speaker diarization** (offline `audio/diarization.rs` + online `audio/online_diarization.rs`, polyvoice ONNX segmentation + ResNet34 embeddings + AHC clustering; `transcripts.speaker`/`speaker_label`, `meetings.diarization_status`/`speaker_names`), **diarization CPU performance optimization** (batched `embed_batch()` inference, parallel stereo channels, chunked long-recording processing, and stage-level timing/memory telemetry), **ffmpeg-streaming diarization decode** (memory-safe 16 kHz f32le pipe streaming in `audio/diarization.rs`, fixed ONNX session pool of `min(8, 75% of cores)`, memory-mode settings removed), **streaming meeting audio player** (`audio/audio_file.rs` + frontend `AudioPlayer`/`useAudioPlayer`), **provider-aware model-readiness gate** (`audio/transcription/commands.rs::check_active_transcription_model_ready`), and **GPU build tooling** (`scripts/copy-cuda-libs.*`, `frontend/build-exe.bat` exe-only build, VS 2026 detection, incremental-build speed tuning). Earlier (since 2026-08-05): mic/system channel separation (stereo, left=mic right=sys; `source_device` on transcripts), VAD rewritten to **Silero v6** with unified `VadConfig` and a rolling buffer (`audio/vad.rs`), transcription provider abstraction (`audio/transcription/`), "Enhance" re-transcription (`audio/retranscription.rs`), audio import (`audio/import.rs`), LLM debug logging (`summary/debug_log.rs`), frontend transcript pagination (`usePaginatedTranscripts.ts`, `VirtualizedTranscriptView.tsx`), and GPU build scripts (`scripts/env-cuda.*`, `frontend/build-gpu.*`/`dev-gpu.*`). The legacy Python `backend/` was removed.
