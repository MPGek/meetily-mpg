...
For any voiceprint row with provenance and a resolvable meeting audio file, the browser SHALL offer playback of the exact source clip: it SHALL stream the meeting recording through the existing media playback path (no full-file IPC transfer), seek to the row's `audio_start_time`, begin playback, and stop at `audio_end_time`. Rows without a resolvable audio file or without timecodes SHALL have playback disabled.
