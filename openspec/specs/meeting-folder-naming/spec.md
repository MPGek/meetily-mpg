# meeting-folder-naming Specification

## Purpose
Meeting recording folders are named with a local-time timestamp so users can identify recordings by when they were made; folder names are deduplicated when multiple recordings start in the same minute.
