...
pub(crate) fn create_transcript_segments(transcripts: &[(String, f64, f64)]) -> Vec<TranscriptSegment> {
    create_transcript_segments_with_source(transcripts, None)
}

/// Create transcript segments with a specific source_device label.
/// Each tuple is (text, start_ms, end_ms) from VAD timestamps.
pub(crate) fn create_transcript_segments_with_source(
