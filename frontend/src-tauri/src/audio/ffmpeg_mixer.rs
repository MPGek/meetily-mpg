...
pub const RNNOISE_APPLY_ENABLED: bool = true; // Default: disabled (Whisper handles noise well)
...
impl FFmpegAudioMixer {
    ...
    pub fn pop_mixed(&mut self, mic_speech_active: bool) -> Option<Vec<f32>> {
        ...
        let mixed = self
            .mixer
            .mix(&mic_samples, &system_samples, mic_speech_active);
...
mod tests {
    ...
    fn test_mixer_no_speech_keeps_system_full() {
        ...
        for sample in mixed {
            assert!(
                (sample - 0.5).abs() < 1e-4,
                "system should stay at full level, got {sample}"
            );
        }
    ...
    fn test_mixer_speech_ducks_system() {
        ...
        assert!(
            (first - 0.5).abs() < 1e-3,
            "gain must ramp (no hard switch), first={first}"
        );
        assert!(
            (last - 0.3).abs() < 0.02,
            "system should be ducked to 0.6 gain, got {last}"
        );
        assert!(
            first > last,
            "gain should decrease smoothly: {first} <= {last}"
        );
    }
    ...
    fn test_mixer_duck_hold_and_release() {
        ...
        assert!(
            (held_last - 0.3).abs() < 0.02,
            "debounce should keep the duck, got {held_last}"
        );
        ...
        assert!(
            (released_last - 0.5).abs() < 0.02,
            "system should return to full, got {released_last}"
        );
    }
    ...
    fn test_mixer_loud_noise_without_speech_not_ducked() {
        ...
        for sample in mixed {
            assert!(
                (sample - 0.7).abs() < 1e-3,
                "system must stay at full level, got {sample}"
            );
        }
        assert!((mixer.current_system_gain - 1.0).abs() < 1e-6);
    }
