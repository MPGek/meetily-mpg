...
# Default name of the project ignore list file (relative to scan root)
DEFAULT_IGNORE_FILE = ".cartographer-ignore"
...
def parse_ignore_file(root: Path, ignore_file_path: Path | None = None) -> set[str]:
    """
    Parse project ignore list file (e.g. .cartographer-ignore).
    Returns a set of directory/file names to exclude from scanning and token estimation.
    One pattern per line; '#' comments and empty lines are skipped. Trailing '/' is stripped.
    """
    if ignore_file_path is not None and ignore_file_path.is_absolute():
        path = ignore_file_path.resolve()
    else:
        path = (root / ignore_file_path) if ignore_file_path else (root / DEFAULT_IGNORE_FILE)
        path = path.resolve()
    names: set[str] = set()
...
def should_ignore(
    ...
    ignore_file_names: set[str] | None = None,
) -> bool:
    """Check if a path should be ignored (excluded from scan and token estimation)."""
    name = path.name
    ignore_names = ignore_file_names or set()

    # Check project ignore list file (folders/paths excluded from token estimation)
...
def scan_directory(
    ...
    Folders listed in the project ignore file (e.g. .cartographer-ignore) are excluded
    from the scan and from token size estimation.
    """
    root = root.resolve()
    gitignore_patterns = parse_gitignore(root)
    ignore_file_names = parse_ignore_file(root, ignore_file_path)
    ...
    def walk(current: Path, depth: int = 0):
        ...
        if should_ignore(current, root, gitignore_patterns, ignore_file_names):
            return
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help=f"Path to ignore list file relative to scan root (default: {DEFAULT_IGNORE_FILE}). "
        "Folders listed there are excluded from token estimation.",
    )
    ...
    result = scan_directory(path, encoding, args.max_tokens, args.ignore_file)
