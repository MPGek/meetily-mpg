...
This is the full version of the Meetily project overview, commands, architecture, patterns, conventions, and operational details.

**Summary**: See [AGENTS.md](../AGENTS.md) for a condensed reference.
...
When fetching webpages or downloading resources from URLs, prefer using `curl` or `wget` over built-in language tools unless a specific reason exists to use something else. **Default to streaming/fetching without saving to disk** — pipe output directly into processing tools (jq, grep, sed, python, etc.) for analysis, extraction, or transformation.

### Streaming Without Saving to Disk (Preferred)

When the goal is to read, parse, or analyze content rather than store it:

```bash
# Fetch JSON and extract fields directly — no file saved
curl -s "https://api.example.com/data" | jq '.key'

# Fetch HTML and grep for specific patterns
curl -sL "https://example.com/page" | grep -o 'pattern'

# Pipe into Python for parsing/processing
curl -sL "https://example.com/api/json" | python3 -c "import sys, json; print(json.load(sys.stdin))"

# Fetch and transform with sed/grep
curl -sL "https://example.com/config.yml" | grep -v '^#' | sed 's/old/new/g'

# Fetch and count results
curl -s "https://api.example.com/search?q=test" | jq '[.[] | select(.status == "active")] | length'

# wget equivalent — stream to stdout by omitting -O
wget -qO- "https://example.com/api/data" | jq '.items'
```

### Saving to Disk (When Needed)

Use these when you need persistent storage:

```bash
# curl with output file
curl -fSL -o output.html "https://example.com/page"

# wget with output file
wget -qO output.html "https://example.com/page"

# Resume large downloads
wget -c "https://example.com/large-file.zip"

# Follow redirects (important for some CDNs)
curl -fSL -o resource.tar.gz "https://download.example.com/resource"
```

**Guidelines**:
- **Default to streaming** — pipe `curl`/`wget` output directly into processors instead of saving files, unless you need persistent storage
- Use `curl` when working in Windows environments (more commonly available by default)
- Use `wget -qO-` for streaming with wget (the `-` means stdout)
- Always use `-f` / `--fail` flag to make it return a non-zero exit code on HTTP errors
- Always use `-L` / `--location` flag to follow redirects
- Prefer `-s` / `-q` (silent) mode for scripting to suppress progress output
- Use `-A` (user-agent) header when some sites block default user agents
- Handle authentication by passing tokens via headers (`-H "Authorization: ..."`) rather than hardcoding URLs

**When applying this rule**:
- Fetching API responses for testing or debugging → stream and parse directly
- Scraping web content for documentation or analysis → pipe to grep/sed/python
- Reading remote configuration files → stream and process in-place
- Downloading Whisper GGML models (`backend/download-ggml-model.*`) → save to disk (persistent storage needed)
- Downloading assets, fonts, or other remote resources during build/dev → save to disk (persistent storage needed)
